    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="mb-1 flex flex-wrap items-center gap-2">
          ...
          >
            <span className="material-symbols-outlined text-xs">{sourceMeta.icon}</span>
            {sourceMeta.label}
          </span>
          ...
          {status !== 'open' && status !== 'done' && !sourceDetail?.stateName && (
            <span className="rounded-full bg-surface-container px-2 py-0.5 text-[11px] font-medium text-on-surface-variant">
        ...
        <div className="flex items-start justify-between gap-2">
          <h4
            className={cn(
              'text-base font-semibold leading-snug text-on-surface',
              isDone && 'line-through',
            )}
          >
            {title}
